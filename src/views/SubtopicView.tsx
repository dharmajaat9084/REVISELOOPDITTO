...
import { Subtopic, AppDatabase } from '../types';
import {
  crumbFor,
  dueCountOf,
  getSub,
...
export const SubtopicView: React.FC<SubtopicViewProps> = ({
  id,
  db,
  onBack,
  onOpenFolder,
  ...
  const crumb = crumbFor('subtopic', s.id);
  const parentName = crumb.length > 1 ? crumb[crumb.length - 2].name : 'Topic';

  const folders = db.folders.filter(
    (f) => f.parentType === 'subtopic' && f.parentId === s.id && !f.isArchived
  );
  const direct = db.questions.filter(
    (q) => q.parentType === 'subtopic' && q.parentId === s.id
  );

  const due = dueCountOf('subtopic', s.id);
  const od = overdueCountOf('subtopic', s.id);
  const qs = questionsIn('subtopic', s.id);
  const logs = logsForType('subtopic', s.id);

  const handleToggleSchedule = () => {
    if (s.schedule) {
      onAskConfirm(
        'Remove revision schedule',
        'Stop scheduling note-revision for this subtopic? History is kept.',
        () => {
          s.schedule = null;
          saveDB();
      ...
      s.schedule = newSchedule(db.settings.srs.easeStart);
      saveDB();
      onToast('Subtopic revision scheduled', 'calendar');
    }
  };
    ...
    <div>
      <div className="pagehead">
        <div style={{ minWidth: 0 }}>
          ...
          <div className="sub">{s.description || 'Subtopic'}</div>
        </div>
        <div className="grow" />
        <button className="btn small soft" onClick={() => onNoteModal('subtopic', s.id)}>
          <Icon name="clip" size={14} /> Attach
        </button>
        <button
          className="ibtn"
          onClick={(e) => onNodeMenu(e, 'subtopic', s.id)}
          title="Options"
        >
          <Icon name="dots" />
        </button>
      ...
      <div className="sectiontitle">
        Questions <span className="grow" />
        <button
          className="btn small soft"
          onClick={() => onAddQuestion({ parentType: 'subtopic', parentId: s.id })}
        >
      ...
      {direct.length === 0 ? (
        <div className="empty card">
          ...
          <p>Add revision prompts you will review from your own notes.</p>
          <button
      ...
      <div className="sectiontitle">Subtopic revision</div>
      {!s.schedule ? (
        <div
          className="card"
          style={{
        ...
        >
          ...
          <div className="rmain">
            ...
            <div className="rmeta">
              Optionally schedule this subtopic as a whole-notes revision item.
            </div>
          </div>
        ...
        >
          ...
          <div className="rmain">
            ...
            <div className="rmeta">
              {renderDueBadge(s.schedule, false)}
              <span>interval {s.schedule.interval}d</span>
              {s.schedule.lastReviewed && (
                <span>last {timeAgo(s.schedule.lastReviewed)}</span>
              )}
            </div>
          </div>
          <button
            className="btn small primary"
            onClick={() => onStartSingle(`subtopic:${s.id}`)}
          >
            Revise
          </button>
          <button
            className="ibtn"
            onClick={() => onReschedule(`subtopic:${s.id}`)}
            title="Reschedule"
          >
